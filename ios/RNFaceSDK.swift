...
class RNFaceSDK: RCTEventEmitter {
    ...
    @objc func startRegistration(
        ...
        case .denied, .restricted:
            reject("E_PERMISSION", "Camera permission not granted", nil)
            return
        case .notDetermined:
            AVCaptureDevice.requestAccess(for: .video) { [weak self] granted in
                if granted {
                    self?.startRegistration(options, resolver: resolve, rejecter: reject)
                } else {
                    reject("E_PERMISSION", "Camera permission denied by user", nil)
                }
            }
            return
        default:
    ...
    @objc func startRecognition(
        ...
        case .notDetermined:
            AVCaptureDevice.requestAccess(for: .video) { [weak self] granted in
                if granted {
                    self?.startRecognition(options, resolver: resolve, rejecter: reject)
                } else {
                    reject("E_PERMISSION", "Camera permission denied by user", nil)
                }
            }
