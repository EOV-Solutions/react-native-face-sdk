...
const FaceSDK = {
  ...
  /**
   * Wait for the current user's enrollment data to finish syncing into local DB.
   * This helps avoid a false negative when checking enrollment immediately after initialize().
   * @param userId - User ID to check (defaults to faceId from initialize)
   * @param options.timeoutMs - Maximum time to wait in milliseconds (default: 5000)
   * @param options.intervalMs - Delay between checks in milliseconds (default: 400)
   */
  async waitForEnrollmentSync(
    userId?: string,
    options: WaitForEnrollmentSyncOptions = {}
  ): Promise<boolean> {
    const effectiveUserId = userId ?? _initData.faceId;
    const timeoutMs = options.timeoutMs ?? 5000;
    const intervalMs = options.intervalMs ?? 400;

    if (!effectiveUserId) {
      return Promise.reject(new Error('userId is required. Pass it as parameter or set faceId in initialize()'));
    }

    if (timeoutMs < 0 || intervalMs <= 0) {
      return Promise.reject(new Error('timeoutMs must be >= 0 and intervalMs must be > 0'));
    }

    const startedAt = Date.now();

    while (true) {
      const enrolled = await FaceSDK.isUserEnrolled(effectiveUserId);
      if (enrolled) {
        return true;
      }
   ...
   * @param options.mode - 'identify' (1:N, default) | 'verify' (1:1)
   * @param options.userId - User ID to verify against (uses faceId from initialize() if not provided)
   */
  startRecognition(options: RecognitionOptions = {}): Promise<RecognitionResult> {
    const mergedOptions: RecognitionOptions = {
      ...options,
      userId: options.userId ?? _initData.faceId,
    };
    return FaceSDKNative.startRecognition(mergedOptions);
  },
