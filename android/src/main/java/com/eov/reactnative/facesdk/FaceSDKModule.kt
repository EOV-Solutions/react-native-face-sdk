...
class FaceSDKModule(reactContext: ReactApplicationContext) :
    ...
    companion object {
        ...
        private const val REQUEST_CAMERA_FOR_REGISTRATION = 2004
        private const val REQUEST_CAMERA_FOR_RECOGNITION = 2005
    }
    ...
    // Pending state for auto-permission-request flow
    private var pendingRegistrationOptions: ReadableMap? = null
    private var pendingRegistrationPromise: Promise? = null
    private var pendingRecognitionOptions: ReadableMap? = null
    private var pendingRecognitionPromise: Promise? = null
    ...
    fun startRegistration(options: ReadableMap, promise: Promise) {
        ...
        if (!hasPermission()) {
            ...
            pendingRegistrationOptions = options
            pendingRegistrationPromise = promise
            requestCameraPermissionInternal(REQUEST_CAMERA_FOR_REGISTRATION)
            return
        }

        launchRegistration(options, promise)
    }

    private fun launchRegistration(options: ReadableMap, promise: Promise) {
        val activity = reactApplicationContext.currentActivity
        if (activity == null) {
            promise.reject("E_NO_ACTIVITY", "Activity is null")
            return
    ...
    fun startRecognition(options: ReadableMap, promise: Promise) {
        ...
        if (!hasPermission()) {
            // Auto-request permission, then retry
            pendingRecognitionOptions = options
            pendingRecognitionPromise = promise
            requestCameraPermissionInternal(REQUEST_CAMERA_FOR_RECOGNITION)
            return
        }

        launchRecognition(options, promise)
    }

    private fun launchRecognition(options: ReadableMap, promise: Promise) {
        val activity = reactApplicationContext.currentActivity
        if (activity == null) {
            promise.reject("E_NO_ACTIVITY", "Activity is null")
            return
    ...
    private fun requestCameraPermissionInternal(requestCode: Int) {
    ...
    override fun onRequestPermissionsResult(
        ...
        when (requestCode) {
            ...
            REQUEST_CAMERA_FOR_REGISTRATION -> {
                if (granted && pendingRegistrationPromise != null) {
                    launchRegistration(pendingRegistrationOptions!!, pendingRegistrationPromise!!)
                } else {
                    pendingRegistrationPromise?.reject("E_PERMISSION", "Camera permission denied by user")
                }
                pendingRegistrationOptions = null
                pendingRegistrationPromise = null
            }

            REQUEST_CAMERA_FOR_RECOGNITION -> {
                if (granted && pendingRecognitionPromise != null) {
                    launchRecognition(pendingRecognitionOptions!!, pendingRecognitionPromise!!)
                } else {
                    pendingRecognitionPromise?.reject("E_PERMISSION", "Camera permission denied by user")
                }
                pendingRecognitionOptions = null
                pendingRecognitionPromise = null
            }
