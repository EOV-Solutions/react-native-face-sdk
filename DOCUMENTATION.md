...
const initSDK = async () => {
  try {
    ...
    if (result.success) {
      ...
      // initialize() có thể trả success trước khi auto-sync embedding hoàn tất.
      // Chờ local DB sync xong để tránh isUserEnrolled trả false âm tính ngay sau init.
      const enrolled = await FaceSDK.waitForEnrollmentSync('user-12345', {
        timeoutMs: 5000,
        intervalMs: 400,
      });
      console.log('👤 User enrolled local:', enrolled);
    } else {
...
const recognizeFace = async () => {
  ...
  if (result.success) {
    if (result.isRecognized) {
      ...
      console.log('⏱️ Timeout hoặc xác thực người thật thất bại');
    } else {
      console.log('⚠️ Không nhận diện được - Có thể chưa đăng ký');
    }
  } else if (result.error === 'Recognition cancelled') {
    console.log('Người dùng đã đóng camera nhận diện');
  } else {
...
| `waitForEnrollmentSync` | `userId?: string, options?` | `boolean` | Chờ auto-sync enrollment sau initialize |
...
interface RecognitionOptions {
  ...
  mode?: 'verify' | 'identify'; // verify = 1:1 mặc định, identify = 1:N
  userId?: string; // User cần verify, mặc định lấy faceId từ initialize()
}
```
...
| Trường hợp | Result |
|------------|--------|
| Hoàn thành flow | `success: true`, `isLive: true` |
| Timeout / liveness fail | `success: true`, `isLive: false` |
| User bấm close/back camera | `success: false`, `error: 'Recognition cancelled'` |
| Lỗi native | `success: false`, `error` |
